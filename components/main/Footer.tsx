"use client"
import { FaLocationArrow } from "react-icons/fa6";
import { motion } from "framer-motion"; 
import { socialMedia } from "@/constants";
import MagicButton from "../ui/MagicButton";

const Footer = () => {
  return (
    <footer className="w-full pt-20 pb-10 overflow-x-hidden overflow-y-hidden" id="contact">
      {/* background grid */}
      <div className="w-full absolute left-0 -bottom-72 min-h-96"></div>

      <div className="flex flex-col items-center text-white px-4">
        <h1 className="heading lg:max-w-[45vw]">
          Ready to take <span className="text-transparent bg-clip-text bg-gradient-to-b from-purple-500 to-cyan-500">your</span> digital
          presence to the next level?
        </h1>
        <p className="text-white-200 md:mt-10 my-5 text-center">
          Reach out to me today and let&apos;s discuss how I can help you
          achieve your goals.
        </p>
          <a href="mailto:sakshatkumar007@gmail.com">
            <MagicButton
              title="Let's get in touch"
              icon={<FaLocationArrow />}
              position="right"
            />
          </a>
      </div>
      <div className="flex mt-16 md:flex-row flex-col gap-3 justify-between items-center px-5">
       
        <div className="flex items-center md:gap-3 gap-6">
          {socialMedia.map((info) => (
            <motion.div
              drag
              dragConstraints={{
                right:10
              }}
              dragSnapToOrigin={true}
              dragTransition={{bounceStiffness:300, bounceDamping:5}}
              key={info.id}
              className="w-10 h-10 cursor-pointer flex justify-center items-center backdrop-filter backdrop-blur-lg saturate-180 bg-opacity-75 bg-black-200 rounded-lg border border-black-300"
            >
              <img src={info.img} alt="icons" width={20} height={20} />
            </motion.div>
          ))}
        </div>
        <p className="md:text-base text-sm md:font-normal font-light text-white">
          Copyright © 2024 Sakshat Kumar
        </p>

      </div>
    </footer>
  );
};

export default Footer;
